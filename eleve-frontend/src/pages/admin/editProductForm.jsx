...
export default function EditProductForm() {
  ...
  const autoGenerateDescription = async () => {
    const currentName = form.getFieldValue("productName") || form.getFieldValue("name");
    const currentCategory = form.getFieldValue("category");
    const currentBrand = "ELEVÉ";
    const currentBenefits = form.getFieldValue("benefits") || [];
    const currentSkinTypes = form.getFieldValue("skinType") || [];
    ...
    const aiLoadingToast = toast.loading("✨ AI is crafting your story...");
    setLoading(true);
    ...
    try {
      const attributes = [
        ...currentBenefits,
        ...currentSkinTypes,
        ...(typeof currentTags === 'string' ? currentTags.split(",") : [])
      ].filter(Boolean);

      const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/ai/generate-description`, {
        name: currentName,
      ...
      if (response.data.success) {
        form.setFieldsValue({ detailedDescription: response.data.description });
        setFormData(prev => ({ ...prev, detailedDescription: response.data.description }));
        toast.success("✨ Story generated successfully!", { id: aiLoadingToast });
      } else {
        toast.error("Failed to generate description.", { id: aiLoadingToast });
      }
    } catch (error) {
      console.error("AI Generation Error:", error);
      toast.error("Failed to connect to AI service.", { id: aiLoadingToast });
    } finally {
      setLoading(false);
    }
  };
            ...
            <Row gutter={16}>
              ...
              <Col span={24}>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-slate-100">Detailed Description</span>
                  <Button 
                    type="dashed" 
                    icon={<span className="text-xl">✨</span>}
                    onClick={autoGenerateDescription}
                    loading={loading}
                    className="border-cyan-500 text-cyan-400 hover:text-cyan-300 hover:border-cyan-400 bg-slate-800"
                  >
